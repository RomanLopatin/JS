 ...
 * на страницу 
 */
function init(postsNumberToLoad) {
 ...
 */
function insertPosts(postsNumberToLoad) {
    ...
    let postsMarkup = '';
    for (let i = 0; i < postsNumberToLoad; i++) {
        postsMarkup += getPostMarkup(i)
    }
    // let postsMarkup = getPostMarkup(0);
    // 3.2 Перед scrollCheck вставьте записанную в postsMarkup разметку
    let divToInsert = document.createElement('div');
    divToInsert.innerHTML = postsMarkup;
    scrollCheck.insertAdjacentElement('beforebegin', divToInsert)
}
